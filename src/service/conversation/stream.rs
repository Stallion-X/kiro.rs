//! 流式事件转换上下文
//!
//! 本模块持有 [`StreamContext`]（Live：实时处理）与 [`BufferedStreamContext`]（Buffered：
//! 等流结束后修正 `input_tokens` 再批量返回）两个上下文。
//!
...
impl StreamContext {
    ...
    pub fn process_kiro_event(&mut self, event: &Event) -> Result<Vec<SseEvent>, FatalKiroError> {
        match event {
